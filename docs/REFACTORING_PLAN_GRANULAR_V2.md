...
- [ ] Todas as rotas autenticadas ativas envolvidas em `ProtectedRoute`
- [ ] Role hardcodes removidos de páginas
- [ ] Logout seguro implementado localmente (estado limpo + redirect, sem endpoint)
- [ ] UnauthorizedPage criada
...
mutationFn: (creds: ILoginRequest) => authService.login(creds.emailOuUsuario, creds.senha),
onSuccess: (response) => {
...
mutationFn: (status: string) => courseService.updateCourseStatus(courseId, status),
...
1. **Checklist de Submissão:** desenvolvedor completou todos os itens
2. **CODE_REVIEW_CHECKLIST.md:** validar Clean Code, SOLID, Segurança, Performance
3. **RFs/RNs:** comportamento implementado alinha-se com contrato
4. **Testes:** 2-3 casos por função (sucesso, erro, edge case)
   ### Fase de Aprovação (2 Code Reviews Mínimo)
1. **CI/CD verde:** tests, lint, type-check, build
2. **Sem conflitos** com `development`
3. **Merge:** Squash or Rebase (manter histórico granular)
4. **Tag de release:** v.X.Y.Z pós-merge em `main`
```
...
| 0.2 | Integrar routes.ts | 🔄 IN_PROGRESS | 1 | 2/5 | Faltam testes para admin role |
    ## 🎯 Próxima Ação
...
3. Escrever teste: sem token → redirect /
    ## 🚦 Bloqueadores
    **Nenhum no momento.**
    ## 💾 Estado das Sessão
...
1. **Validar com time backend:** Confirmar que todos endpoints listados existem ou estão planejados
2. **Priorizar:** Há alguma fase com maior urgência de negócio? (Recomendação: Fase 0 → Fase 1 → Fase 2)
3. **Se autorizar a execução da Fase 0:** Branch `feature/refactor-auth-guard-global`, 1º commit é ProtectedRoute
4. **Atualizar MEMORY.MD session:** Registrar progresso da sessão ( status inicial, próxima subtarefa)
5. **Commits granulares:** Cada subtarefa = 1-2 commits com rastreabilidade a RNF/RF/RN
6. **Revisão peer:** PR review obrigatório antes de merge em development
---
