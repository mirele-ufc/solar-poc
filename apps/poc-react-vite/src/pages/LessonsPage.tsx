...
export function LessonsPage() {
  ...
  const mod = modId ?? "1";
  const aulas = aulasPorModulo[mod] ?? aulasPorModulo["1"];
  const moduloTitle = moduloTitles[mod] ?? "Módulo 01";

  // Which aula index are we on? Passed via navigation state or default to 0
  const initialIndex: number =
    typeof (location.state as { aulaIndex?: number })?.aulaIndex === "number"
      ? (location.state as { aulaIndex: number }).aulaIndex
      : 0;
  ...
  // Mark current aula visited on mount / when it changes
  useEffect(() => {
    setVisited((prev) => {
      if (prev.has(currentAula.id)) return prev;
      const next = new Set(prev);
      next.add(currentAula.id);
      saveVisited(next);
      return next;
    });
  }, [currentAula.id]);
    ...
    <main id="main-content" className="bg-white flex flex-col min-h-screen">
      ...
      <div className="flex-1 overflow-y-auto pb-[96px]">
        <div className="max-w-[600px] mx-auto flex flex-col gap-[20px] px-[16px] md:px-[24px] pt-[20px] w-full">
          ...
          {/* Module card with image + lesson title */}
          <div className="bg-[#c5d6ff] rounded-[12px] w-full overflow-hidden flex flex-col gap-[15px] px-[18px] pt-[18px] pb-[18px]">
            {/* Lesson image */}
          ...
          <p
            className="font-['Figtree:Regular',sans-serif] font-normal text-[14px] leading-[20px] text-[#8e8e8e]"
            aria-live="polite"
          >
          ...
          >
            {currentAula.body.map((paragraph, i) => (
              <p
                key={i}
                className="font-['Figtree:Regular',sans-serif] font-normal text-[16px] leading-[24px] text-black"
              >
                {paragraph}
              </p>
            ))}
      ...
      {/* ── Fixed bottom button ── */}
      <div className="fixed bottom-0 left-0 right-0 z-20 bg-white flex items-center justify-center px-[18px] py-[16px] shadow-[0px_0px_12px_0px_rgba(51,51,51,0.2)]">
        <button
          type="button"
          onClick={handleNext}
