  ...
  return (
    <div className="w-full mt-[4px]">
      <div className="flex justify-between items-center mb-[4px]">
        <span className="font-['Figtree:Regular',sans-serif] font-normal text-[#021b59] text-[12px] leading-[18px]"
      ...
      <div className="w-full h-[6px] bg-[#759BFB]/40 rounded-full overflow-hidden"
        role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100}
        aria-label="Progresso do módulo">
        <div className="h-full bg-[#042e99] rounded-full transition-all duration-300" style={{ width: `${pct}%` }} />
      </div>
...
// ── Declaração de Matrícula Modal ─────────────────────────────────────────────

function DeclaracaoMatriculaModal({
  onClose, studentName, validationCode, emissionDate,
}: {
  onClose: () => void;
  studentName: string;
  ...
  return (
    <>
      ...
      <div
        ...
        role="dialog" aria-modal="true" aria-labelledby="matricula-modal-title"
      >
        <div className="bg-white w-full max-w-[580px] rounded-[12px] shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="bg-[#021b59] px-[28px] py-[20px] flex items-center justify-between">
            <h2 id="matricula-modal-title"
              className="font-['Figtree:Bold',sans-serif] font-bold text-white text-[18px] leading-[28px]">
              Declaração de Matrícula
            </h2>
            <button type="button" onClick={onClose} aria-label="Fechar declaração de matrícula"
              className="text-white/80 hover:text-white transition-colors focus-visible:outline focus-visible:outline-[2px] focus-visible:outline-white focus-visible:rounded-[4px] p-[4px]">
          ...
          <div className="px-[28px] md:px-[40px] py-[32px] flex flex-col gap-[20px]">
            ...
            <div className="font-['Figtree:Regular',sans-serif] text-[#333] text-[15px] leading-[26px] text-justify">
              <p>
                ...
                <span className="font-['Figtree:Bold',sans-serif] font-bold text-[#021b59]">Python Iniciante</span>,
...
// ── Declaração de Conclusão Modal ─────────────────────────────────────────────

function DeclaracaoConclusaoModal({
  onClose, studentName, validationCode, emissionDate, conclusionDate,
}: {
  onClose: () => void;
  studentName: string;
  validationCode: string;
  ...
  return (
    <>
      ...
      <div
        className="fixed inset-0 z-50 flex items-center justify-center px-[16px] py-[24px] overflow-y-auto"
        role="dialog" aria-modal="true" aria-labelledby="conclusao-modal-title"
      >
        <div className="bg-white w-full max-w-[580px] rounded-[12px] shadow-2xl flex flex-col overflow-hidden">

          {/* Header */}
          <div className="bg-[#042e99] px-[28px] py-[20px] flex items-center justify-between">
            <h2 id="conclusao-modal-title"
              className="font-['Figtree:Bold',sans-serif] font-bold text-white text-[18px] leading-[28px]">
              Declaração de Conclusão
            </h2>
            <button type="button" onClick={onClose} aria-label="Fechar declaração de conclusão"
              className="text-white/80 hover:text-white transition-colors focus-visible:outline focus-visible:outline-[2px] focus-visible:outline-white focus-visible:rounded-[4px] p-[4px]">
              <svg className="size-[22px]" fill="none" viewBox="0 0 24 24" aria-hidden="true">
                <path d="M18 6L6 18M6 6l12 12" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
              </svg>
...
function RequisitosAlert({
  totalAulas, visitedAulas, totalProvas, visitedProvas, onClose,
}: {
  totalAulas: number; visitedAulas: number;
  totalProvas: number; visitedProvas: number;
  onClose: () => void;
}) {
  useEffect(() => {
    function handleEsc(e: KeyboardEvent) { if (e.key === "Escape") onClose(); }
  ...
  return (
    <>
      ...
      <div
        ...
        aria-labelledby="requisitos-title" aria-describedby="requisitos-desc"
      >
        <div className="bg-white w-full max-w-[420px] rounded-[12px] shadow-2xl p-[28px] flex flex-col gap-[20px]">
          ...
          <div className="flex flex-col gap-[10px]">
            {[
              { label: "Aulas concluídas",  done: visitedAulas  >= totalAulas,  detail: `${visitedAulas} de ${totalAulas} aulas` },
              { label: "Provas realizadas", done: visitedProvas >= totalProvas, detail: `${visitedProvas} de ${totalProvas} provas` },
            ].map(({ label, done, detail }) => (
              <div key={label}
                className={`flex items-center gap-[12px] px-[14px] py-[10px] rounded-[10px] ${done ? "bg-[#c5d6ff]/40" : "bg-[#ffeac4]/60"}`}>
                <div className={`size-[22px] rounded-full flex items-center justify-center shrink-0 ${done ? "bg-[#042e99]" : "bg-[#D45900]"}`} aria-hidden="true">
                  {done ? (
...
export function PythonModulosPage() {
  ...
  // Stable codes generated once per session
  ...
  const isConcluded   = visitedAulas >= ALL_AULA_IDS.length && visitedProvas >= ALL_PROVA_IDS.length;
  ...
  return (
    <main id="main-content" className="bg-white flex flex-col min-h-screen pb-[110px]">
      <div className="max-w-[900px] mx-auto flex flex-col gap-[20px] px-[16px] md:px-[40px] pt-[20px] w-full">
        ...
        {/* ── Botão Declaração de Matrícula + progresso geral ── */}
                ...
                <div className="bg-[#c5d6ff] rounded-[12px] w-full overflow-hidden">
                  <div className="flex flex-col gap-[12px] px-[16px] pt-[16px] pb-[16px]">
                    ...
                    <div className="w-full aspect-[16/9] rounded-[10px] overflow-hidden">
                      <img
                        alt={`Imagem representativa do ${mod.title}`}
                    ...
                    <button
                      ...
                      aria-controls={`modulo-${mod.id}-content`}
                    ...
                    >
                      <span className="font-['Figtree:Bold',sans-serif] font-bold text-[22px] leading-[32px] text-black">
                        {mod.title}
                      </span>
                      <ArrowIcon up={isOpen} />
                    </button>
                    ...
                    {isOpen && (
                      <div id={`modulo-${mod.id}-content`} className="flex flex-col">
                        {mod.items.map((item, idx) => {
                          ...
                          return (
                            <div key={item.id}>
                              <button
                                type="button"
                                onClick={() => handleVisitLesson(item.id, item.type)}
                              ...
                              {!isLast && <div className="w-full h-px bg-[#759BFB]" aria-hidden="true" />}
                            </div>
                          );
                        })}
                      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white px-[16px] md:px-[40px] py-[14px] shadow-[0px_-4px_12px_rgba(51,51,51,0.12)] z-10">
        <div className="max-w-[900px] mx-auto">
          ...
          >
            ...
            <span className={`font-['Figtree:Medium',sans-serif] font-medium text-[17px] ${isConcluded ? "text-white" : "text-[#333]"}`}>
              Declaração de Conclusão
            </span>
            {!isConcluded && (
