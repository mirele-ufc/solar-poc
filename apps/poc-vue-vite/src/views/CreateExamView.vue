...
// Importe seu store se necessário, ou use props/estado local conforme o original
...
const config = reactive<ConfigSettings>({
  wrongAnswers: false,
  rightAnswers: false,
  values: false,
});
    ...
    <div v-if="!editorOpen" class="max-w-[900px] mx-auto flex flex-col gap-[20px] px-[20px] md:px-[40px] pt-[30px] w-full">
      ...
      <div v-for="mod in modulesList" :key="mod.id" class="border border-black rounded-[4px] overflow-hidden">
        <div class="p-[16px] flex flex-col gap-[16px]">
          ...
          <button @click="openEditor(mod.id)" class="bg-[#ffeac4] h-[60px] w-full rounded-[26px] font-medium text-[20px] hover:bg-[#ffd9a0] mt-2 transition-colors">
            {{ (moduleProvas[mod.id]?.length || 0) > 0 ? "Editar prova" : "Adicionar prova" }}
          </button>
        </div>
      </div>
