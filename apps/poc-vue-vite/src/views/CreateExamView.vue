...
type Question = { id: string; text: string; options: { id: string; text: string }[]; correctOptionId: string; points: number };
...
<template>
  <div class="bg-white min-h-screen flex flex-col pb-[100px]">
    ...
    <div v-if="!editorOpen" class="max-w-[900px] mx-auto flex flex-col gap-[20px] px-[20px] md:px-[40px] pt-[30px] w-full">
      ...
      <div v-for="mod in modulesList" :key="mod.id" class="border border-black rounded-[4px] overflow-hidden">
        <div class="p-[16px] flex flex-col gap-[16px]">
          ...
          <div v-if="moduleProvas[mod.id] && moduleProvas[mod.id].length > 0" class="bg-[#c5d6ff] h-[60px] flex items-center justify-between px-[20px] rounded mb-2">
            <p class="text-[20px] font-medium text-black truncate flex-1">
              Prova do Módulo 
              <span class="text-[16px] font-normal text-[#555] ml-1">
                ({{ moduleProvas[mod.id].length }} {{ moduleProvas[mod.id].length === 1 ? 'questão' : 'questões' }})
              </span>
            </p>
            
            <div class="flex items-center gap-[12px]">
              <button @click="openEditor(mod.id)" class="size-[24px] hover:scale-110 transition-transform">
                <svg class="size-full" viewBox="0 0 24 24"><path :d="EDIT_ICON" fill="#021b59" /></svg>
              </button>
              <button @click="deleteQuiz(mod.id)" class="size-[24px] hover:scale-110 transition-transform">
                <svg class="size-full" viewBox="0 0 24 24"><path :d="CLOSE_SM" fill="#801436" /></svg>
              </button>
            </div>
          </div>
          ...
          <div class="flex flex-col sm:flex-row items-center gap-[12px] mt-2">
            <button 
              ...
              :disabled="moduleProvas[mod.id] && moduleProvas[mod.id].length > 0"
    ...
    <ManualExamEditor 
      ...
      @close="editorOpen = false; editingModId = null"
