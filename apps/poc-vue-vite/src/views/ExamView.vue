...
const confirmSubmit = () => {
  router.push({
    path: "/courses/power-bi/exam/results",
...
<template>
  <div class="bg-white min-h-screen pb-[60px]">
    ...
    <div class="max-w-[900px] mx-auto flex flex-col gap-[28px] px-[20px] md:px-[40px] pt-[30px]">
      <PageHeader
        title="Prova 01"
      ...
      <div v-else class="flex flex-col gap-[28px]">
        <p
          ...
          style="font-size: clamp(22px, 5vw, 34px); font-variation-settings: 'wdth' 100;"
        >
          Power BI - Fundamentos
        </p>

        <div class="flex items-center justify-between border-b border-[#e0e0e0] pb-[16px]">
          <h2 class="font-['Figtree:Bold',sans-serif] font-bold text-black text-[26px] leading-tight">
            Prova 01
          </h2>
          <div
          ...
          >
            <svg class="size-[20px] shrink-0" fill="none" viewBox="0 0 24 24" aria-hidden="true">
              <path fill-rule="evenodd" clip-rule="evenodd" :d="clockPath" fill="#595959" />
            </svg>
            <span class="font-['Figtree:Medium',sans-serif] font-medium text-[#595959] text-[17px] tabular-nums">
              {{ formatTime(timeLeft) }}
            </span>
          </div>
        </div>
          ...
          <legend class="float-left w-full">
            <div class="flex items-start justify-between gap-[12px]">
              <div class="flex-1 min-w-0">
                ...
                <p class="font-['Figtree:Medium',sans-serif] font-medium text-black text-[17px] leading-[26px]">
                  {{ q.text }}
                </p>
              </div>
              <div class="shrink-0 bg-[#f5f5f5] px-[14px] py-[6px] rounded-[8px]">
                <span class="font-['Figtree:Medium',sans-serif] font-medium text-[#595959] text-[15px] whitespace-nowrap">
                  1,0 pt
                </span>
              </div>
            </div>
            ...
            >
              ...
              <div
                :class="[
                  ...
                  answers[q.id] === idx ? 'bg-[#ffeac4] border-[#021b59]' : 'bg-white border-[#aaa]',
                ]"
                aria-hidden="true"
              >
                <svg v-if="answers[q.id] === idx" class="size-[13px]" fill="none" viewBox="0 0 22 22">
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M5 9L3 11L9 17L19 7L17 5L9 13L5 9Z" fill="#021B59" />
                </svg>
              </div>
              <span class="font-['Figtree:Medium',sans-serif] font-medium text-[15px] text-[#021b59] shrink-0">
                {{ optionLabels[idx] }}
              </span>
              <span class="font-['Figtree:Regular',sans-serif] font-normal text-[16px] text-black leading-[24px] flex-1">
                {{ opt }}
              </span>
            </label>
          </div>
    ...
    >
      <div class="bg-white rounded-[12px] w-full max-w-[460px] p-[28px] shadow-xl">
        <h2
          id="confirm-title"
        ...
        <div class="flex flex-col sm:flex-row gap-[12px] sm:gap-[16px]">
          ...
          >
            <span class="font-['Figtree:Medium',sans-serif] font-medium text-[#021b59] text-[18px]">
              Cancelar
            </span>
          </button>
          <button
          ...
          >
            <span class="font-['Figtree:Medium',sans-serif] font-medium text-white text-[18px]">
              Enviar respostas
            </span>
          </button>
        </div>
