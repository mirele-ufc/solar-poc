...
const enrolled = computed(() => courseStore.isEnrolledInCourse("python"));
const showCancelModal = ref(false);

const handleInscrever = () => {
  if (enrolled.value) router.push("/courses/python/modules");
  else router.push("/courses/python/enrollment");
};

const handleConfirmCancel = () => {
  courseStore.unenrollFromCourse("python");
    ...
    <div class="max-w-[900px] mx-auto flex flex-col gap-[24px] px-[20px] md:px-[40px] pt-[24px] w-full">
      ...
      <div class="flex flex-col gap-[10px]">
        ...
        <p class="font-['Figtree:Regular',sans-serif] font-normal leading-[26px] text-black text-[16px]">
          O curso Python Iniciante tem como objetivo introduzir os participantes ao universo da programação utilizando a linguagem Python — uma das mais populares e versáteis do mundo. Ao longo do curso, os alunos aprenderão desde os conceitos básicos de lógica de programação até a construção de pequenos programas funcionais, desenvolvendo uma base sólida para avançar em áreas como ciência de dados, automação e desenvolvimento de software.
        </p>
      </div>
      ...
      <div class="flex flex-col gap-[10px]">
        ...
        <ul class="flex flex-col gap-[8px] pl-[4px]">
          <li v-for="item in publicoAlvo" :key="item" class="flex gap-[8px] items-start">
            <span aria-hidden="true" class="text-[#021b59] mt-[2px]">•</span>
            <span class="font-['Figtree:Regular',sans-serif] font-normal leading-[24px] text-black text-[16px]">
              {{ item }}
            </span>
      ...
      <div class="flex flex-col gap-[10px]">
        ...
        <ul class="flex flex-col gap-[8px] pl-[4px]">
          <li v-for="item in preRequisitos" :key="item" class="flex gap-[8px] items-start">
            <span aria-hidden="true" class="text-[#021b59] mt-[2px]">•</span>
            <span class="font-['Figtree:Regular',sans-serif] font-normal leading-[24px] text-black text-[16px]">
              {{ item }}
            </span>
          ...
          <div class="bg-white w-full max-w-[420px] rounded-[12px] shadow-2xl p-[28px] flex flex-col gap-[20px]">
            ...
            <div class="flex flex-col gap-[10px]">
              <button
                ...
                class="w-full h-[50px] bg-[#de2e66] rounded-[26px] hover:bg-[#ba348a] transition-colors focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-[#de2e66] focus-visible:outline-offset-[2px] font-['Figtree:Medium',sans-serif] font-medium text-white text-[17px]"
              >
                Confirmar cancelamento
              </button>
              <button
                @click="showCancelModal = false"
                class="w-full h-[50px] border-2 border-[#021b59] rounded-[26px] hover:bg-[#021b59]/5 transition-colors focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-[#021b59] focus-visible:outline-offset-[2px] font-['Figtree:Medium',sans-serif] font-medium text-[#021b59] text-[17px]"
                autoFocus
              >
