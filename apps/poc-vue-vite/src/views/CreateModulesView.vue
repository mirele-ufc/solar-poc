...
const validateModules = () => {
  if (modules.value.length === 0) {
    error.value = "Por favor, adicione pelo menos um módulo para finalizar o cadastro";
...
const addModule = async () => {
  ...
  try {
    const createdModule = await moduleService.createModule(courseIdReal, defaultName);
...
const handleAddLesson = (lessonId: number, name: string, file: string | null) => {
      ...
      >
        <div class="flex flex-col gap-[14px] p-[16px] pb-0">
          ...
          <button
            ...
            :class="mod.image ? '' : 'bg-[#f0f0f0] border-2 border-dashed border-[#8e8e8e] hover:bg-[#e8e8e8]'"
          >
            ...
            <template v-else>
              <svg class="size-[32px] mb-[6px]" fill="none" viewBox="0 0 24 24" aria-hidden="true">
                <path :d="imgIconPath" stroke="#595959" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
              <p class="font-['Figtree:Regular',sans-serif] text-[14px] text-[#595959]">
          ...
          >
            ...
            <div class="flex items-center gap-[6px] shrink-0 ml-[10px]">
              ...
              <button @click="removeLesson(mod.id, lesson.id)" type="button" class="shrink-0 size-[44px] flex items-center justify-center focus-visible:outline focus-visible:outline-[2px] focus-visible:outline-[#021b59] rounded">
                <svg class="size-[25px]" fill="none" viewBox="0 0 28 28" aria-hidden="true">
