...
const handleFinalizar = async () => {
  const missing =
    !nome.value.trim() || !sobrenome.value.trim() || !cidade.value.trim() || !genero.value;
  ...
  // Garantir que o scroll aconteça após a renderização do banner de sucesso
  await nextTick();
  topRef.value?.scrollIntoView({ behavior: "smooth" });
...
const toggleGenero = () => {
  generoOpen.value = !generoOpen.value;
};
...
<template>
  <div class="bg-white flex flex-col pb-[100px]">
    ...
    <div class="max-w-[900px] mx-auto flex flex-col gap-[16px] px-[20px] md:px-[40px] pt-[24px] w-full">
      <PageHeader
        title="Inscrição"
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-[16px]">
        <div class="flex flex-col w-full gap-[2px]">
          ...
          <div :class="['bg-white h-[60px] w-full rounded-[12px] relative border flex items-center px-[20px] transition-all', (showErrors && !nome.trim()) ? 'border-2 border-[#c0392b]' : 'border-[#5f5f5f] focus-within:outline focus-within:outline-[3px] focus-within:outline-[#021b59] focus-within:outline-offset-[-1px]']">
        ...
        <div class="flex flex-col w-full gap-[2px]">
          ...
          <div :class="['bg-white h-[60px] w-full rounded-[12px] relative border flex items-center px-[20px] transition-all', (showErrors && !sobrenome.trim()) ? 'border-2 border-[#c0392b]' : 'border-[#5f5f5f] focus-within:outline focus-within:outline-[3px] focus-within:outline-[#021b59] focus-within:outline-offset-[-1px]']">
      ...
      <div class="flex flex-col w-full gap-[2px]">
        ...
        <div :class="['bg-white h-[60px] w-full rounded-[12px] relative border flex items-center px-[20px] transition-all', (showErrors && !cidade.trim()) ? 'border-2 border-[#c0392b]' : 'border-[#5f5f5f] focus-within:outline focus-within:outline-[3px] focus-within:outline-[#021b59] focus-within:outline-offset-[-1px]']">
      ...
      <div class="flex flex-col w-full gap-[2px]">
        ...
        <div class="relative">
          <button
            ...
            @click="toggleGenero"
            :class="['bg-white h-[60px] w-full rounded-[12px] flex items-center gap-[10px] px-[20px] py-[12px] relative transition-all border', (showErrors && !genero) ? 'border-2 border-[#c0392b]' : 'border border-[#5f5f5f]']"
          >
            ...
            <svg class="size-[13px] shrink-0 transition-transform duration-200" fill="none" viewBox="0 0 13 10.03" aria-hidden="true" :style="{ transform: generoOpen ? 'rotate(180deg)' : 'none' }">
          ...
          <div v-if="generoOpen">
            ...
            <ul role="listbox" aria-label="Gênero" class="absolute left-0 right-0 top-[60px] bg-white border border-[#5f5f5f] z-30 shadow-lg overflow-hidden rounded-b-[12px]">
              <li v-for="g in generos" :key="g">
                <button type="button" role="option" :aria-selected="genero === g" @click="selectGenero(g)" class="w-full text-left px-[20px] py-[14px] font-['Figtree:Regular',sans-serif] font-normal text-[16px] text-[#333] hover:bg-[#ffeac4]/40 focus-visible:outline focus-visible:outline-[2px] focus-visible:outline-[#021b59]">
                  {{ g }}
                </button>
              </li>
            </ul>
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white px-[20px] py-[16px] shadow-[0px_-4px_12px_rgba(51,51,51,0.15)] flex justify-center z-10">
      <div class="w-full max-w-[900px]">
        <button
          ...
          class="bg-[#ffeac4] h-[50px] w-full rounded-[26px] cursor-pointer hover:bg-[#ffd9a0] transition-colors focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-[#021b59] focus-visible:outline-offset-[2px] disabled:opacity-60 disabled:cursor-not-allowed"
        >
