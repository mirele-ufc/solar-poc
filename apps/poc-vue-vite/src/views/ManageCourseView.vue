...
const COURSES_MOCK: Record<string, CourseMock> = {
  "power-bi": {
    title: "Power BI - Fundamentos",
    image:
      "https://images.unsplash.com/photo-1759661966728-4a02e3c6ed91?fit=max&fm=jpg&w=1080",
    hours: "30h",
    totalStudents: 35,
    avgScore: 72,
    modules: [
...
<template>
  <div
    class="bg-white flex flex-col pb-[80px] min-h-screen font-['Figtree',sans-serif]"
  >
    <div class="w-full h-[180px] md:h-[220px] overflow-hidden relative">
      <img :src="currentCourse.image" class="w-full h-full object-cover" />
      <div
      ...
      <button
        @click="router.back()"
        class="absolute top-[16px] left-[16px] size-[36px] rounded-full bg-white/80 flex items-center justify-center hover:bg-white transition-colors"
      >
        <svg class="size-[20px]" fill="none" viewBox="0 0 24 24">
      ...
      <div class="absolute bottom-[16px] left-[20px] right-[20px]">
        ...
        <p class="text-white/80 text-[14px]">
          Carga horária: {{ currentCourse.hours }} ·
          {{ currentCourse.totalStudents }} alunos inscritos
        </p>
      </div>
    </div>

    <div
      class="max-w-[900px] mx-auto w-full px-[20px] md:px-[40px] pt-[20px] flex flex-col gap-[20px]"
    >
      <div class="flex flex-col sm:flex-row gap-[10px]">
        <button
          @click="showParticipants = true"
          class="flex-1 flex items-center justify-center gap-[8px] h-[46px] bg-[#ffeac4] rounded-[26px] hover:bg-[#ffd9a0] transition-colors"
        >
          <svg class="size-[18px]" fill="none" viewBox="0 0 24 24">
            <path :d="PEOPLE_PATH" fill="#333" />
          </svg>
          <span class="font-medium text-[#333] text-[16px]"
        ...
        <button
          ...
          class="flex-1 flex items-center justify-center gap-[8px] h-[46px] bg-[#021b59] rounded-[26px] hover:bg-[#042e99] transition-colors disabled:opacity-60"
        >
          <svg
            v-if="!generatingPDF"
            class="size-[18px]"
          ...
          <span class="font-medium text-[#ffeac4] text-[16px]">{{
        ...
        <button
          ...
          class="flex-1 flex items-center justify-center h-[46px] border-2 border-[#021b59] rounded-[26px] hover:bg-[#021b59]/5 transition-colors"
        >
          <span class="font-medium text-[#021b59] text-[16px]"
            >Enviar comunicado</span
          >
        </button>
      </div>
      ...
      <div class="flex gap-0 border-b border-[#e0e0e0]">
        <button
          ...
          :class="[
            'px-[20px] py-[10px] font-medium text-[15px] border-b-2 transition-colors',
            activeTab === 'dashboard'
      ...
      <div v-if="activeTab === 'dashboard'" class="flex flex-col gap-[20px]">
        <div class="grid grid-cols-2 md:grid-cols-3 gap-[12px]">
          <div
            class="flex flex-col gap-[6px] rounded-[12px] px-[16px] py-[14px] bg-[#c5d6ff]"
          >
            <p class="font-bold text-[28px] text-[#021b59]">
        ...
        <div
          class="bg-[#ffeac4] rounded-[12px] px-[16px] py-[24px] flex items-center justify-between"
        >
          <div>
            <p class="font-bold text-[32px] text-[#021b59]">
              {{ currentCourse.avgScore }}%
            </p>
            <p class="font-medium text-[#333] text-[15px]">
              Média de acertos nas provas
            </p>
          </div>
          <div
            class="size-10 flex items-center justify-center bg-white/50 rounded-lg"
          >
        ...
        <div class="flex flex-col gap-[12px]">
          <h3 class="font-bold text-[#801436] text-[18px]">
            Alunos que pararam — onde estavam
          </h3>
          <div
            v-for="student in stoppedStudents"
            :key="student.id"
            class="flex flex-col gap-1 p-[14px] bg-[#fde8ef] rounded-[10px]"
          >
            <div class="flex justify-between items-start">
              <div>
                ...
                <p class="text-[#801436] text-[13px]">
                  Parou em: {{ student.stoppedAt }}
                </p>
              </div>
              <div class="text-right">
                <p class="font-bold text-[#801436]">{{ student.progress }}%</p>
                <p class="text-[#8e8e8e] text-[11px]">
                  {{ student.lastAccess }}
                </p>
              </div>
            </div>
      ...
      <section v-if="activeTab === 'modulos'" class="flex flex-col gap-[16px]">
        <p
          ...
          class="font-['Figtree:Regular',sans-serif] text-[#8e8e8e] text-[15px] text-center py-[24px]"
        >
        ...
        <div
          ...
          :key="mod.id"
          class="border border-[#e0e0e0] rounded-[8px] overflow-hidden"
        >
          <!-- Module header -->
          <div
            class="flex items-center justify-between gap-[8px] bg-[#021b59] px-[16px] py-[12px]"
          >
            <p
              class="font-['Figtree:Bold',sans-serif] font-bold text-white text-[16px] flex-1 min-w-0 truncate"
            >
              {{ mod.name }}
            </p>
          ...
          <div class="flex flex-col divide-y divide-[#e8e8e8]">
            <p
              ...
              class="font-['Figtree:Regular',sans-serif] text-[#8e8e8e] text-[14px] px-[16px] py-[12px]"
            >
            ...
            <div
              ...
              :key="lesson.id"
              class="bg-[#c5d6ff] flex items-center justify-between px-[16px] py-[14px] gap-[8px]"
            >
              <button
                type="button"
    ...
    >
      ...
      <div
        class="bg-white w-full max-w-[520px] rounded-[16px] shadow-2xl flex flex-col max-h-[85vh] z-50 overflow-hidden"
      >
        <div
          class="flex items-center justify-between px-[24px] py-[18px] border-b border-[#e0e0e0] shrink-0"
        >
          <h2 class="font-bold text-[#021b59] text-[20px]">
            Participantes ({{ students.length }})
          </h2>
          <button
            @click="showParticipants = false"
            class="size-[32px] flex items-center justify-center hover:opacity-70"
          >
            <svg class="size-[20px]" fill="none" viewBox="0 0 24 24">
              <path :d="CLOSE_SM" fill="#333" />
            </svg>
          </button>
        </div>
        <div class="flex-1 overflow-y-auto">
          <div
            ...
            :key="student.id"
            class="px-[24px] py-[14px] border-b border-[#e0e0e0] last:border-b-0 hover:bg-[#f9f9f9]"
          >
            <div class="flex items-start justify-between gap-3">
              ...
              <div class="text-right shrink-0">
                <p class="font-bold text-[#021b59] text-[15px]">
                  {{ student.progress }}%
                </p>
                <p class="text-[#8e8e8e] text-[11px]">
