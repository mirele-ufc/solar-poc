...
const { allCourses, isLoading, errorMessage, coursesByCategory, fetchCourses } =
  useCourses();
...
<template>
  <div class="bg-white min-h-screen">
    <div
      v-if="isProfessor"
      class="flex flex-col gap-[30px] items-start pt-[30px] px-[20px] md:px-[40px] pb-[60px]"
    >
      <div class="flex flex-col gap-[12px] w-full">
        <h1
          class="font-['Figtree:Bold',sans-serif] font-bold leading-[36px] text-[#021b59] text-[24px]"
        >
          Acompanhe os cursos
        </h1>
        <button
      ...
      <div
        ...
        class="w-full bg-[#fee] border border-[#f99] rounded-[8px] p-[16px]"
      >
      ...
      <template v-if="!isLoading && !errorMessage && allCourses.length > 0">
        <section class="flex flex-col gap-[20px] w-full">
          <h2
            class="font-['Figtree:Bold',sans-serif] font-bold text-[#021b59] text-[24px]"
          >
            Cursos ativos
          </h2>
          <CourseGrid
            ...
            @click-course="handleManage"
          />
        </section>
      </template>

      <div
        v-if="!isLoading && !errorMessage && allCourses.length === 0"
    ...
    >
      <h1
        class="font-['Figtree:Bold',sans-serif] font-bold text-[#021b59] text-[28px]"
      >
