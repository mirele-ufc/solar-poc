...
const ALL_COURSES: Record<string, CourseInfo> = {
  "power-bi": {
...
const studentCourses = computed(() => {
  const studentCourseIds = isProfessor
    ? courseStore.enrolledCourses.filter((id: string) =>
        courseStore.courseStudentRoles.includes(id),
      )
    : courseStore.enrolledCourses;

  return studentCourseIds
    .map((id: string) => ({ id, ...(ALL_COURSES[id] ?? null) }))
    .filter((c): c is { id: string } & CourseInfo => "title" in c && !!c.title);
});
...
<template>
  <div
    v-if="!isProfessor"
    class="bg-white flex flex-col gap-[24px] pt-[24px] px-[20px] md:px-[40px] pb-[60px]"
  >
    <PageHeader
      title="Meus Cursos"
      backPath="/courses"
    ...
    <div
      ...
      class="flex flex-col items-center gap-[20px] py-[60px] text-center"
    >
      <svg
        class="size-[64px] opacity-30"
        fill="none"
        viewBox="0 0 24 24"
        aria-hidden="true"
      >
      ...
      <div class="flex flex-col gap-[8px]">
        ...
        <p
          class="font-['Figtree:Regular',sans-serif] font-normal text-[#595959] text-[15px]"
        >
      ...
      >
        <span
          class="font-['Figtree:Medium',sans-serif] font-medium text-[#333] text-[17px]"
        >
    ...
    <div
      v-else
      class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-[20px]"
    >
      <CourseCard
  ...
  <div
    v-else
    class="bg-white flex flex-col gap-[32px] pt-[24px] px-[20px] md:px-[40px] pb-[60px]"
  >
    <PageHeader
      title="Meus Cursos"
      backPath="/courses"
    ...
    <section aria-labelledby="secao-professor">
      <div class="flex items-center gap-[10px] mb-[16px]">
        <div
          class="size-[32px] rounded-full bg-[#021b59] flex items-center justify-center shrink-0"
          aria-hidden="true"
        >
          <svg
            class="size-[17px]"
    ...
    <section aria-labelledby="secao-aluno">
      <div class="flex items-center gap-[10px] mb-[16px]">
        <div
          class="size-[32px] rounded-full bg-[#042e99] flex items-center justify-center shrink-0"
          aria-hidden="true"
        >
          <svg
            class="size-[17px]"
      ...
      <div
        v-if="studentCourses.length === 0"
        class="flex flex-col items-center gap-[16px] py-[40px] text-center bg-[#f5f8ff] rounded-[12px]"
      >
        <svg
          class="size-[48px] opacity-25"
          fill="none"
          viewBox="0 0 24 24"
          aria-hidden="true"
        >
        ...
        <p
          class="font-['Figtree:Regular',sans-serif] font-normal text-[#595959] text-[14px]"
        >
          Explore os cursos disponíveis e realize sua inscrição.
        </p>
        <button
