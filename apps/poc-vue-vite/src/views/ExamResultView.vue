...
<template>
  <div class="bg-white min-h-screen pb-[60px]">
    ...
    <div v-else-if="allCorrect" class="max-w-[900px] mx-auto flex flex-col items-center gap-[28px] px-[20px] md:px-[40px] pt-[40px]">
      ...
      <div class="flex flex-col items-center justify-center size-[160px] rounded-full bg-[#042e99] shadow-lg" aria-label="Resultado: 100%">
        <span class="font-['Anek_Devanagari:ExtraBold',sans-serif] font-extrabold text-[#ffeac4] text-[42px] leading-none">
          100%
        </span>
      ...
      <div class="flex flex-col items-center gap-[8px] text-center">
        <h1 class="font-['Anek_Devanagari:ExtraBold',sans-serif] font-extrabold text-[#021b59] leading-tight text-[clamp(24px,5vw,34px)]">
        ...
        <div v-for="(q, qIdx) in questions" :key="q.id" class="border border-[#28a745] rounded-[12px] p-[16px] bg-[#f0faf3] flex flex-col gap-[10px]">
          <div class="flex items-start justify-between gap-[12px]">
            <div class="flex-1 min-w-0">
              <p class="font-['Figtree:Bold',sans-serif] font-bold text-black text-[18px] leading-tight">Questão {{ qIdx + 1 }}</p>
          ...
          <div class="flex flex-col gap-[6px]">
            <div v-for="(opt, idx) in q.options" :key="idx" 
              ...
              <span v-if="idx === q.correctIndex" class="shrink-0 text-[12px] font-['Figtree:Medium',sans-serif] text-[#155724] bg-[#d4edda] px-[8px] py-[2px] rounded-full whitespace-nowrap">
      ...
      <button @click="handleReturn" class="bg-[#042e99] h-[52px] w-full rounded-[26px] cursor-pointer hover:bg-[#021b59] transition-colors text-[#ffeac4] font-['Figtree:Medium',sans-serif] text-[20px]">
        Retornar ao curso
      </button>
    </div>

    <div v-else class="max-w-[900px] mx-auto flex flex-col gap-[28px] px-[20px] md:px-[40px] pt-[30px]">
      <h1 class="font-['Anek_Devanagari:ExtraBold',sans-serif] font-extrabold leading-tight text-black text-[clamp(22px,5vw,34px)]">
        Power BI - Fundamentos
      </h1>
      <h2 class="font-['Figtree:Bold',sans-serif] font-bold text-black text-[24px] leading-tight -mt-[16px]">
        Prova 01 — Resultado
      </h2>
      ...
      <div v-for="(q, qIdx) in questions" :key="q.id" class="flex flex-col gap-[14px] border-b border-[#e0e0e0] pb-[28px]">
        <div class="flex items-start justify-between gap-[12px] flex-wrap">
          <div class="flex-1 min-w-0">
            <p class="font-['Figtree:Bold',sans-serif] font-bold text-black text-[20px] leading-tight">Questão {{ qIdx + 1 }}</p>
        ...
        <div class="flex flex-col gap-[6px]">
          <div v-for="(opt, idx) in q.options" :key="idx"
            ...
            <span v-else-if="idx === userAnswers[q.id]" class="shrink-0 text-[12px] font-['Figtree:Medium',sans-serif] text-white bg-[#e74c3c] px-[8px] py-[2px] rounded-full whitespace-nowrap">
              Sua resposta
            </span>
          </div>
        </div>
        ...
        <div class="bg-[#f5f5f5] px-[16px] py-[10px] rounded-[8px]">
          <p class="font-['Figtree:Regular',sans-serif] text-black text-[15px] leading-[24px]">
            <strong>Gabarito questão {{ qIdx + 1 }}:</strong> {{ q.correctLabel }}) {{ q.options[q.correctIndex] }}
          </p>
        </div>
      </div>

      <button @click="handleReturn" class="bg-[#ffeac4] h-[52px] w-full rounded-[26px] cursor-pointer hover:bg-[#ffd9a0] transition-colors text-[#333] font-['Figtree:Medium',sans-serif] text-[20px]">
        Retornar ao curso
      </button>
    </div>
  </div>
