...
const saveExam = async () => {
  ...
  try {
    ...
    formError.value = error.message || "Erro ao salvar a prova no servidor. Tente novamente.";
  } finally {
...
<template>
  <div class="max-w-[900px] mx-auto w-full px-[20px] md:px-[40px] pt-[20px] flex flex-col">
    <div class="flex items-center justify-between mb-[16px]">
      <p class="font-bold text-[#021b59] text-[18px]">Adicionar prova</p>
      <button @click="emit('close')" class="text-[#801436] font-medium text-[14px] hover:underline">
        Cancelar e Voltar
      </button>
    </div>
    
    <div v-if="formError" class="mb-4 p-3 bg-red-100 border border-red-400 text-red-700 rounded text-center font-medium">
      {{ formError }}
    </div>
    ...
    <div v-if="activeTab === 'Perguntas'" class="flex flex-col gap-5">
      ...
      <div class="flex gap-4 items-end mt-2">
        ...
        <button class="size-[50px] rounded-full bg-[#ffeac4] flex items-center justify-center shrink-0"><svg class="size-8" viewBox="0 0 50 50"><path :d="IMG_ICON" fill="black" /></svg></button>
      </div>
      ...
      <div class="flex flex-col gap-2">
        ...
        <button @click="options.push({ id: uid(), text: '' })" class="text-[#021b59] self-start mt-1 font-medium">+ Adicionar alternativa</button>
      </div>

      <div class="flex flex-col gap-1">
        <p class="font-medium text-[18px]">Resposta correta</p>
        <select v-model="correctOptionId" class="w-full h-[50px] border border-[#8e8e8e] rounded px-4 bg-white outline-none">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          <button @click="points = Math.max(1, points - 1)" class="size-7 rounded-full border border-[#021b59] flex items-center justify-center font-bold text-[#021b59] hover:bg-[#f5f8ff]">−</button>
          <span class="w-5 text-center font-medium">{{ points }}</span>
          <button @click="points++" class="size-7 rounded-full border border-[#021b59] flex items-center justify-center font-bold text-[#021b59] hover:bg-[#f5f8ff]">+</button>
        </div>
      </div>

      <button @click="addQuestion" class="w-full border-2 border-[#021b59] rounded-[26px] h-[50px] flex items-center justify-center gap-2 hover:bg-[#f5f8ff] transition-colors mt-2">
        <svg class="size-7" viewBox="0 0 32 32"><path :d="DOC" fill="#021B59" /></svg>
        <span class="font-medium text-[20px] text-[#021b59]">Adicionar pergunta</span>
      </button>
    </div>
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white px-[20px] py-[16px] shadow-[0px_-4px_12px_rgba(51,51,51,0.2)] flex justify-center z-10">
      ...
      >
        <svg v-if="isSavingQuiz" class="animate-spin -ml-1 mr-3 h-5 w-5 text-[#021b59]" fill="none" viewBox="0 0 24 24">
