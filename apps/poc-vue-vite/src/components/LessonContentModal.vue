...
const paragraphs = MOCK_LESSON_CONTENT.split("\n\n");
</script>

<template>
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-lg max-w-2xl w-full max-h-[80vh] flex flex-col shadow-lg">
      <!-- Header -->
      <div class="flex items-center justify-between gap-4 px-6 py-4 border-b border-[#e0e0e0]">
        <h2 class="font-['Figtree:Bold',sans-serif] font-bold text-[#021b59] text-[18px] truncate">
          {{ lessonName }}
        </h2>
        <button
          type="button"
          aria-label="Fechar"
          @click="emit('close')"
          class="shrink-0 p-1 hover:bg-[#f0f0f0] rounded transition-colors focus-visible:outline focus-visible:outline-[2px] focus-visible:outline-[#021b59]"
        >
          <X class="size-5 text-[#021b59]" />
        </button>
      </div>
      ...
      <div class="flex-1 overflow-y-auto px-6 py-4">
        <div class="prose prose-sm max-w-none">
          <p
            ...
            :key="idx"
            class="font-['Figtree:Regular',sans-serif] text-[#333333] text-[15px] leading-[1.6] mb-4"
          >
            {{ paragraph }}
          </p>
        </div>
      ...
      <!-- Footer -->
      <div class="border-t border-[#e0e0e0] px-6 py-4 flex justify-end gap-3">
        <button
          type="button"
          @click="emit('close')"
          class="h-[40px] px-6 font-['Figtree:Medium',sans-serif] font-medium text-[15px] rounded-[8px] bg-[#021b59] text-white hover:bg-[#021b59]/90 transition-colors focus-visible:outline focus-visible:outline-[2px] focus-visible:outline-[#021b59]"
        >
          Fechar
        </button>
      </div>
